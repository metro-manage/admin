...
export default ()=>{
    ...
    const { containerItem, elementItem, elementItemCalendar, elementItemList, buttonAdd, elementItemCalendarData, elementItemListData, buttonOption, btnOpenCalendar, textYearMonth } = elements

    let elementOption = null
    let elementForm   = null

    const confirmDelete = modalConfirmation({ message : '¿Eliminar?' }, ()=> {
        modalAlert.show({ message : 'no disponible por el momento', name : 'danger' })
        // apiProducto().remove()
        //     .then(res => {
    ...
    const renderDataDate =(date = null)=>{
        ...
        elementItemCalendarData.innerHTML = [...Array(datetime.end).keys() ].map( (day, index) => {
    ...
    const dataRender =( data = null )=>{
        ...
        if( !data ) {
            return ele.append( main, elementNotFound({
                href  : '#/asistencia',
                title : '',
        ...
        renderDataDate()
        ele.append( containerItem, elementItem )
    ...
    const dataRenderList =( Data = [] )=>{
        ...
        // ele.append( elementItemList, ele.append( elementItemListData, fragment ) )
        ele.append( elementItemList, elementItemListData )
    }
